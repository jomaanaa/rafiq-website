...
function providerPhotoUrl(?string $photo): string {
    $photo = trim((string)$photo);
    if ($photo === '') return '';

    $photo = str_replace(chr(92), '/', $photo);
    ...
    // Correct path from /patient/doc_service.php to /pictures/providers/
    if ($baseName !== '') {
        return '../pictures/providers/' . $baseName;
    }
...
function specialtyIcon(string $speciality): string {
    $map = [
        'Cardiology (heart)'                   => 'fa-heart-pulse',
        'Neurology (brain & nerves)'          => 'fa-brain',
        'Psychiatry (mental health)'          => 'fa-head-side-virus',
        'Gastroenterology (digestive system)' => 'fa-notes-medical',
        'Pediatrics'                          => 'fa-child-reaching',
        'Orthopedics'                         => 'fa-bone',
    ];
    return $map[$speciality] ?? 'fa-stethoscope';
}
...
| Doctors
|--------------------------------------------------------------------------
| Same user flow as interpreter, but for doctor speciality:
| 1) Fixed specialities from doctor signup
| 2) Choose speciality
| 3) Doctors from DB appear under that speciality
|--------------------------------------------------------------------------
*/
// ── Specialities from DB — same pattern as interpreter languages ────────────
// Each doctor row has a single speciality string. We collect them all,
// deduplicate by lowercase key, and sort case-insensitively — exactly
// the same way int_service.php builds its $languages list.
// canonicalDoctorSpeciality: normalises casing variants stored in the DB
// e.g. "cardiology (heart)", "Cardiology (Heart)", "Cardiology (heart)"
// all become "Cardiology (heart)" so they deduplicate into a single entry.
function canonicalDoctorSpeciality(string $value): string {
    $v = trim($value);
    if ($v === '') return '';
    // Lowercase the whole thing first, then Title-case the first letter only.
    // This gives "Cardiology (heart)", "Neurology (brain & nerves)" etc.
...
try {
    $stmtSpec = $pdo->query("
        SELECT d.speciality
        FROM doctor d
        INNER JOIN provider p ON p.user_id = d.user_id
        WHERE COALESCE(p.status, 'pending') = 'accepted'
          AND d.speciality IS NOT NULL
          AND TRIM(d.speciality) <> ''
    ");
    $rowsSpec = $stmtSpec->fetchAll(PDO::FETCH_ASSOC);

    $specSet = [];
    foreach ($rowsSpec as $row) {
        $raw = trim((string)($row['speciality'] ?? ''));
        if ($raw === '') continue;
        // Normalise before deduplication so "Cardiology (Heart)" and
        // "Cardiology (heart)" both map to the same lowercase key.
        $sp  = canonicalDoctorSpeciality($raw);
        $key = mb_strtolower($sp);
        if (!isset($specSet[$key])) {
            $specSet[$key] = $sp;
        }
    }

    natcasesort($specSet);
    $specialities = array_values($specSet);
} catch (Exception $e) {
    $specialities = ['General'];
}

$selected_speciality = trim((string)($_POST['speciality'] ?? $_GET['speciality'] ?? ''));

// Also normalise the posted value so it matches the canonical list
if ($selected_speciality !== '') {
    $selected_speciality = canonicalDoctorSpeciality($selected_speciality);
    if (!in_array($selected_speciality, $specialities, true)) {
        $selected_speciality = '';
    }
}

$providerColumns = tableColumns($pdo, 'provider');
$providerPhotoSelect = in_array('photo', $providerColumns, true) ? "p.photo AS provider_photo," : "NULL AS provider_photo,";
...
try {
    ...
    foreach ($rows as $row) {
        ...
        $speciality = canonicalDoctorSpeciality((string)($row['speciality'] ?? ''));

        if ($speciality === '') continue;

        // Auto-register any speciality that appeared in DB but wasn't in our list
        // (handles race conditions between the two queries)
        if (!in_array($speciality, $specialities, true)) {
            $specialities[] = $speciality;
            $doctors[$speciality] = [];
        }

        $name = trim(($row['first_name'] ?? '') . ' ' . ($row['last_name'] ?? ''));
        if ($name === '') $name = 'Doctor #' . $id;

        $gender = strtolower(trim((string)($row['gender'] ?? 'male')));

        $img = providerPhotoUrl($row['user_photo'] ?? '');
        if ($img === '') {
            $img = providerPhotoUrl($row['provider_photo'] ?? '');
        }
        if ($img === '') {
...
if (isset($_GET["action"]) && $_GET["action"] === "doctor_day_slots") {
    ...
    try {
        ...
        echo json_encode([
            ...
            "provider_icon" => $doctorIconById[$provider_id] ?? "fa-stethoscope",
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["submit_booking"])) {
    ...
    $speciality = trim((string)(postv("speciality") ?? ''));
    ...
    if ($speciality === "") {
        $submit_error = "Please choose a speciality.";
    ...
    if ($submit_error === "") {
        try {
            ...
            if (!$doctorRow) {
                ...
                $doctorSpeciality = canonicalDoctorSpeciality((string)($doctorRow['speciality'] ?? ''));
                // Compare trimmed values — the posted $speciality came directly from our
                // DB-sourced list so this should always match.
                if ($doctorSpeciality !== trim($speciality)) {
                    $submit_error = "Selected doctor does not match the chosen speciality.";
                }
            }
        } catch (Exception $e) {
    ...
    if ($submit_error === "") {
        ...
        try {
            ...
            $add('service_type', ':service_type', 'Doctor');
            $add('comment', ':comment', 'Speciality: ' . $doctorSpeciality);
...
<style>
.doctor-top img[style*="display: none"] + div,
.selected-doctor-content img[style*="display: none"] + div{
...
<div class="page-wrap">
    ...
    <section class="top-box">
        <div class="top-content">
            ...
            <p>Choose the specialty first, then select your doctor and pick one available appointment slot. Choose the duration that suits your visit.</p>
            ...
            <div class="steps-flow">
                <div class="flow-step active" id="stepIndicator1">
                    ...
                    <div class="flow-icon"><i class="fa-solid fa-stethoscope"></i></div>
                ...
                <div class="flow-step" id="stepIndicator2">
                    ...
                    <div class="flow-icon"><i class="fa-solid fa-user-doctor"></i></div>
    ...
    <form method="POST" id="bookingForm">
        ...
        <input type="hidden" name="speciality" id="specialityHidden" value="">
        ...
        <section id="step1Section" class="simple-card">
            ...
            <div class="type-grid">
                <?php foreach($specialities as $type): ?>
                    <label class="type-card <?= $selected_speciality === $type ? 'active' : '' ?>">
                        <input type="radio" name="speciality" value="<?= h($type) ?>" <?= $selected_speciality === $type ? 'checked' : '' ?> required>
                        <span class="specialty-label">
                            <span class="specialty-icon"><i class="fa-solid <?= h(specialtyIcon($type)) ?>"></i></span>
                            <span><?= h($type) ?></span>
                        </span>
                    </label>
                <?php endforeach; ?>
        ...
        <section id="step2Section" class="simple-card">
            ...
            <div class="small-help">You can search by doctor name, filter by gender, or sort by rating.</div>
            ...
            <div class="booking-layout">
                ...
                <div class="side-stack">
                    ...
                    <div style="display:none;">
                        ...
                        <div id="selectedDoctorPreview" class="selected-doctor-empty">No doctor selected yet.</div>
                    </div>
                </div>
            </div>
        </section>
...
const doctorsData = <?= json_encode($doctors, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
const selectedSpecialityFromServer = <?= json_encode($selected_speciality, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
...
const paymentTotalInput = null; // no dedicated hidden field; total is computed server-side
...
let selectedProviderIcon = "fa-stethoscope";
...
function getSelectedTotal(){
    const units = Math.max(1, Math.ceil(getSelectedDuration() / 30));
    return units * 150;
}

function calculateTotalForDuration(minutes){
    const units = Math.max(1, Math.ceil(minutes / 30));
    return units * 150;
}
...
function updatePricePreview(){
    ...
    if(pricePreview){
        pricePreview.textContent = `EGP ${total}`;
    }
        ...
        container.innerHTML += `
            <label class="doctor-card ${selectedClass}">
                <input
                    ...
                    data-img="${doc.img}"
                    data-icon="${escapeHtml(doc.icon || 'fa-stethoscope')}"
                    ${checked}
                >

                <div class="doctor-top">
                    <img src="${doc.img}" alt="${escapeHtml(doc.name)}" onerror="this.onerror=null;this.src='data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 180 180%22><rect width=%22180%22 height=%22180%22 rx=%2234%22 fill=%22%23eef1f8%22/><text x=%2290%22 y=%2298%22 text-anchor=%22middle%22 font-size=%2254%22 fill=%22%23404066%22>Dr</text></svg>';">
                    <div>
                        <div class="doctor-name">${escapeHtml(doc.name)}</div>
                        <div class="doctor-sub">
                            <span><i class="fa-solid ${escapeHtml(doc.icon || 'fa-stethoscope')}"></i></span>
                ...
                <div class="doctor-stats">
                    <span class="stat-pill"><i class="fa-solid fa-star"></i> ${doc.rating}</span>
...
function renderSelectedDoctorPreview(){
    ...
    selectedDoctorPreview.innerHTML = `
        <div class="selected-doctor-content">
            <img src="${selectedProviderImg}" alt="${escapeHtml(selectedProviderName)}" onerror="this.onerror=null;this.src='data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 180 180%22><rect width=%22180%22 height=%22180%22 rx=%2234%22 fill=%22%23eef1f8%22/><text x=%2290%22 y=%2298%22 text-anchor=%22middle%22 font-size=%2254%22 fill=%22%23404066%22>Dr</text></svg>';">
            <div>
                <div class="selected-doctor-name">${escapeHtml(selectedProviderName)}</div>
                <div class="selected-doctor-meta">
                    <div><i class="fa-solid ${escapeHtml(selectedProviderIcon)}"></i> ${escapeHtml(selectedProviderType)}</div>
                    <div><i class="fa-solid fa-star"></i> ${escapeHtml(selectedProviderRating)}</div>
...
async function chooseDoctor(radio){
    ...
    selectedProviderIcon = radio.dataset.icon || "fa-stethoscope";
    ...
    for(let i=1; i<=daysInMonth; i++){
        ...
        if(dateStr < TODAY){
            ...
            d.onclick = async function(){
                if(!selectedProviderId){
                    showBookingError("Please choose a doctor first.");
...
function renderSlotsFromLastData(){
    ...
    const filteredAvailable = available.filter(slot => {
        ...
        if(selectedDateInput.value === SERVER_NOW_DATE && start <= nowMinutes) {
            return false;
        }

        for(const bookedSlot of booked){
            const bStart = timeToMinutesJs(bookedSlot.from);
            const bEnd = timeToMinutesJs(bookedSlot.to);
    ...
    if(!filteredAvailable.length){
        availableSlotsEl.innerHTML = `<div class="no-slots">No available start times for ${escapeHtml(formatDuration(duration))} on this date.</div>`;
    } else {
        availableSlotsEl.innerHTML = filteredAvailable.map(slot => {
            const end = addMinutesToTime(slot.from, duration);
            return `
    ...
    if(!booked.length){
        ...
        bookedSlotsEl.innerHTML = booked.map(slot => {
            ...
            return `<div class="booked-card">
                <div class="booked-name">${escapeHtml(selectedProviderName || 'Doctor')}</div>
...
async function refreshDoctorDaySlots(){
    ...
    const url = apiUrl({ action:"doctor_day_slots", date: dateStr, provider_id: selectedProviderId });
    const res = await fetch(url, { cache:"no-store" });
    const data = await res.json();
    ...
    if(!data || data.error){
        ...
        availableSlotsEl.innerHTML = `<div class="no-slots">Could not load available slots.</div>`;
        bookedSlotsEl.innerHTML = `<div class="no-slots">No data.</div>`;
        updateSelectionSummary();
        return;
    }

    lastSlotsData = data;
    slotsTitle.innerHTML = `<i class="fa-solid ${escapeHtml(data.provider_icon || 'fa-stethoscope')}"></i> ${escapeHtml(cleanProviderName(data.provider_name, selectedProviderName || 'Doctor'))} Schedule`;
...
document.querySelectorAll('input[name="speciality"]').forEach(radio => {
    radio.addEventListener("change", async function(){
        clearBookingError();

        currentType = this.value;
        specialityHidden.value = this.value;
        selectedPrice = 150;
        ...
        selectedProviderImg = "";
        selectedProviderIcon = "fa-stethoscope";

        providerIdHidden.value = "";
        selectedDateInput.value = "";
        bookingTimeHidden.value = "";
        serviceTimeHidden.value = "";
        durationMinutesHidden.value = "";

        resetSelectedSlot();
        renderSelectedDoctorPreview();
        renderDoctors(currentType);
        await refreshCalendar();
        await refreshDoctorDaySlots();
        updateTopSteps();
        scrollToSection("step2Section");
...
document.getElementById("bookingForm").addEventListener("submit", function(e){
    ...
    if(!currentType){
        e.preventDefault();
        showBookingError("Please choose a speciality first.");
...
currentType = selectedSpecialityFromServer || "";
specialityHidden.value = currentType;
selectedPrice = 150;
updatePricePreview();
renderDoctors(currentType);
renderSelectedDoctorPreview();
...
if (localStorage.getItem("rafiq_scroll_to_doctors") === "1") {
    localStorage.removeItem("rafiq_scroll_to_doctors");
    if (currentType) {
...
window.addEventListener('load', function(){
    ...
    if (typeof renderCalendar === 'function' && daysEl && (!daysEl.children.length || !String(monthEl?.textContent || '').trim())) {
