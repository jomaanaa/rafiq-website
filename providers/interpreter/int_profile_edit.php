...
$_SESSION['provider_id'] = $interpreter_id;
...
function fetchProfile(PDO $pdo, int $id): ?array {
    $stmt = $pdo->prepare("
        ...
        INNER JOIN interpreter i ON i.user_id = p.user_id
        WHERE p.user_id = :id
        LIMIT 1
    ");
    $stmt->execute([':id' => $id]);
...
$error = "";
$data = fetchProfile($pdo, $interpreter_id);

if (!$data) {
    die("Interpreter profile not found.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($first_name === '' || $last_name === '' || $email === '' || $phone === '' || $address === '' || $gender === '' || $languages === '') {
        ...
        try {
            ...
            if (!empty($_FILES['photo']['name']) && is_uploaded_file($_FILES['photo']['tmp_name'])) {
                ...
                $fileName = 'interpreter_' . $interpreter_id . '_' . time() . '.' . $ext;
            ...
            if (!empty($_FILES['cv']['name']) && is_uploaded_file($_FILES['cv']['tmp_name'])) {
                ...
                $cvFileName = 'cv_interpreter_' . $interpreter_id . '_' . time() . '.' . $cvExt;
            ...
            $updateInterpreter = $pdo->prepare("
                UPDATE interpreter
                SET languages = :languages
                WHERE user_id = :id
            ");
            $updateInterpreter->execute([
                ':languages' => $languages,
                ':id' => $interpreter_id
            ]);
            ...
            header("Location: int_profile.php");
            exit;
...
$initials = strtoupper(substr($data['first_name'] ?? 'I', 0, 1) . substr($data['last_name'] ?? '', 0, 1));
...
<title>Edit Interpreter Profile</title>
    ...
    <section class="profile-card">
        <div class="profile-hero">
            <div class="hero-content">
                <div class="avatar">
                    <?php if ($photo !== ''): ?>
                        <img src="<?= h($photoSrc) ?>" alt="<?= h($fullName ?: 'Interpreter photo') ?>">
                    <?php else: ?>
                        <?= h($initials) ?>
                    <?php endif; ?>
                </div>
                ...
                <div>
                    ...
                    <p class="profile-sub"><?= h($fullName ?: 'Interpreter Profile') ?></p>
                </div>
            </div>
        </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    <div class="form-box">
                        ...
                        <input type="text" id="first_name" name="first_name" required value="<?= h($data['first_name'] ?? '') ?>">
                    </div>

                    <div class="form-box">
                        <label for="last_name">Last Name</label>
                        <input type="text" id="last_name" name="last_name" required value="<?= h($data['last_name'] ?? '') ?>">
                    </div>

                    <div class="form-box">
                        <label for="email">Email</label>
                        <input type="email" id="email" name="email" required value="<?= h($data['email'] ?? '') ?>">
                    </div>

                    <div class="form-box">
                        <label for="phone">Phone Number</label>
                        <input type="text" id="phone" name="phone" required value="<?= h($data['phone'] ?? '') ?>">
                    </div>

                    <div class="form-box">
                        <label for="dob">Date of Birth</label>
                        <input type="date" id="dob" name="dob" value="<?= h($data['dob'] ?? '') ?>">
                    </div>
                    ...
                    <div class="form-box wide">
                        ...
                        <textarea id="address" name="address" required><?= h($data['address'] ?? '') ?></textarea>
                    </div>

                    <div class="form-box">
                        <label for="languages">Languages</label>
                        <input type="text" id="languages" name="languages" required value="<?= h($data['languages'] ?? '') ?>">
                        <div class="note">Separate multiple languages with commas.</div>
                    </div>
                    ...
                    <div class="form-box wide">
                        ...
                        <?php if (!empty($data['cv'])): ?>
                            <div class="note">
                                Current CV:
                                <a href="../../uploads/<?= h(basename($data['cv'])) ?>" target="_blank" rel="noopener">View uploaded CV</a>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="actions">
                    <a class="btn cancel" href="int_profile.php">Cancel</a>
