...
function fetchProfile(PDO $pdo, int $id): ?array {
    $stmt = $pdo->prepare("
        SELECT
            ...
            , d.speciality, d.medical_license
        FROM provider p
        INNER JOIN \"user\" u ON u.user_id = p.user_id
        INNER JOIN doctor d ON d.user_id = p.user_id
        WHERE p.user_id = :id
        LIMIT 1
    ");
    $stmt->execute([':id' => $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $medicalLicensePath = trim((string)($data['medical_license'] ?? ''));
    ...
    if ($first_name === '' || $last_name === '' || $email === '' || $phone === '' || $address === '' || $gender === '') {
        ...
        $error = "Please enter your speciality.";
    }  else {
        try {
            ...
            if (!empty($_FILES['photo']['name']) && is_uploaded_file($_FILES['photo']['tmp_name'])) {
                ...
                $fileName = 'doctor_' . $provider_id . '_' . time() . '.' . $ext;
            ...
            if (!empty($_FILES['cv']['name']) && is_uploaded_file($_FILES['cv']['tmp_name'])) {
                ...
                $cvFileName = 'cv_doctor_' . $provider_id . '_' . time() . '.' . $cvExt;
            ...
            if (!empty($_FILES['medical_license']['name']) && is_uploaded_file($_FILES['medical_license']['tmp_name'])) {
                $allowedLicense = ['pdf', 'jpg', 'jpeg', 'png'];
                $licenseExt = strtolower(pathinfo($_FILES['medical_license']['name'], PATHINFO_EXTENSION));

                if (!in_array($licenseExt, $allowedLicense, true)) {
                    throw new Exception("Medical license must be PDF, JPG, JPEG, or PNG.");
                }

                if ((int)$_FILES['medical_license']['size'] > 5 * 1024 * 1024) {
                    throw new Exception("Medical license must be less than 5MB.");
                }

                $licenseName = 'medical_license_doctor_' . $provider_id . '_' . time() . '.' . $licenseExt;
                $licenseTarget = $cvUploadDir . $licenseName;

                if (!move_uploaded_file($_FILES['medical_license']['tmp_name'], $licenseTarget)) {
                    throw new Exception("Could not upload medical license.");
                }

                $medicalLicensePath = $cvUploadUrl . $licenseName;
            }
    ...
    <section class="profile-card">
        <div class="profile-hero">
            <div class="hero-content">
                <div class="avatar">
                    <?php if ($photo !== ''): ?>
                        <img src="<?= h($photoSrc) ?>" alt="<?= h($fullName ?: 'Doctor photo') ?>">
                    <?php else: ?>
                        <?= h($initials) ?>
                    <?php endif; ?>
                </div>
                ...
                <div>
                    ...
                    <p class="profile-sub"><?= h($fullName ?: 'Doctor Profile') ?></p>
                </div>
            </div>
        </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    ...
                    <div class="form-box">
                        ...
                        <input type="text" id="speciality" name="speciality" required value="<?= h($data['speciality'] ?? '') ?>">
                    </div>
                    <div class="form-box">
                        <label for="medical_license">Medical License</label>
                        <input type="file" id="medical_license" name="medical_license" accept=".pdf,.jpg,.jpeg,.png">
                        <?php if (!empty($data['medical_license'])): ?>
                            <div class="note">Current: <a href="../../uploads/<?= h(basename($data['medical_license'])) ?>" target="_blank" rel="noopener">View medical license</a></div>
                        <?php endif; ?>
                    </div>
                    <div class="form-box">
                ...
                <div class="actions">
                    <a class="btn cancel" href="doctor_profile.php">Cancel</a>
