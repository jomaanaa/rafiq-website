...
if(!isset($_SESSION['step1']) || !isset($_SESSION['step2']) || $_SESSION['provider_type'] !== 'caregiver'){
...
if(isset($_POST['finish'])){
    ...
    $shift = trim($_POST['shift_preference']);
    ...
    if(empty($shift)){
        ...
        $insert_caregiver = pg_query_params(
            $conn,
            'INSERT INTO caregiver 
            (user_id,shift_preference)
             VALUES ($1,$2)',
            array($user_id,$shift)
        );

        if(!$insert_caregiver){
            die("Caregiver insert failed: " . pg_last_error($conn));
        }
...
<head>
<meta charset="UTF-8">
<title>Rafiq - Caregiver Sign Up</title>
...
<div class="card">
    ...
    <div class="subtitle">
        Join Rafiq and provide care services.<br>
    ...
    <div class="question">Shift Preference</div>
    <div class="small">Select your preferred working shift</div>
