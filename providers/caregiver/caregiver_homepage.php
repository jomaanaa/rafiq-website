...
$provider_name = get_provider_name($pdo, $provider_id, 'Caregiver');
...
$serviceWhere = "";
if (True) { $serviceWhere = " AND (LOWER(COALESCE(service_type,'')) = 'caregiver' OR service_type IS NULL) "; }
...
<title>Caregiver Dashboard | Rafiq</title>
    ...
    <section class="hero-card">
        <div class="hero-top">
            <div>
                ...
                <p class="hero-sub">Review new requests, manage confirmed care sessions, and keep track of completed work.</p>
            </div>
    ...
    <section class="dashboard-grid">
            ...
            <div class="list">
                <?php if (!$pendingBookings): ?><div class="empty">There are no new booking requests at the moment.</div><?php else: ?>
                    <?php foreach ($pendingBookings as $b): ?>
                        <?php render_booking_card($b, [ ['action'=>'accept','class'=>'primary','icon'=>'fa-check','label'=>'Accept'], ['action'=>'reject','class'=>'danger','icon'=>'fa-xmark','label'=>'Decline'] ], ['Service'=>($b['service_type'] ?? 'Caregiver')]); ?>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
        </div>
        <div class="stack">
            <div class="panel compact-card">
                <div class="panel-head"><div><h2 class="panel-title">Current Care Sessions</h2><p class="panel-sub">Confirmed bookings and work in progress.</p></div><div class="count-pill"><?= h(count($activeBookings)) ?></div></div>
                <div class="list">
                    <?php if (!$activeBookings): ?><div class="empty">You do not have any active bookings right now.</div><?php else: ?>
                        <?php foreach ($activeBookings as $b): ?>
                            <?php
                                ...
                                if (in_array(($b['status'] ?? ''), ['accepted','arrived'], true)) $actions[] = ['action'=>'start','class'=>'primary','icon'=>'fa-play','label'=>'Start Care'];
            ...
            <div class="panel compact-card">
                <div class="panel-head"><div><h2 class="panel-title">Completed Care Sessions</h2><p class="panel-sub">Most recent finished bookings.</p></div><div class="count-pill"><?= h(count($history)) ?></div></div>
                <div class="list">
